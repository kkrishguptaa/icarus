...
export class Play extends Scene {
  ...
  create() {
    ...
    this.player = new Player(this, this.levelManager.current.start.x, this.levelManager.current.start.y);
      ...
      const resetMsg = this.add
        .text(
          ...
          '💀 Icarus has fallen once again! 💀\nBut it\'s a game so you respawn! from level one though...',
          {
            ...
            strokeThickness: 12,
            align: 'center',
            wordWrap: { useAdvancedWrap: true, width: this.cameras.main.width - 100 },
          },
        )
        .setOrigin(0.5)
        .setDepth(15);
  ...
  setupCollisions() {
    ...
    const hazards = this.levelManager.hazards.getChildren() as Phaser.GameObjects.GameObject[];
    hazards.forEach((hazard) => {
      // Check if hazard has a physics body
      const physicsObj = hazard as Phaser.Physics.Arcade.Sprite | Phaser.GameObjects.Rectangle;
      if (physicsObj) {
    ...
    if (this.levelManager.portal) {
      ...
      const portalChildren = this.levelManager.portal.getAll() as Phaser.Physics.Arcade.Sprite[];
      const portalSprite = portalChildren.find(child => child.texture?.key === 'portal');
